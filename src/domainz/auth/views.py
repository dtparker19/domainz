...
from domainz.auth.forms import (
    LoginForm,
    RegisterForm,
    UpdatePasswordForm,
    UpdateUserForm,
)
from domainz.auth.utils import validate_google_id_token
from domainz.billing.utils import create_subscription_for_user
...
def account_settings_view(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            return redirect("domainz-auth:account-settings")
    ...
    if request.method == "POST":
        if form.is_valid():
            if request.user.check_password(form.cleaned_data["password"]):
                ...
                return redirect("domainz-auth:security-settings")
            else:
...
def signin_with_google_view(request):
    ...
    try:
        ...
        if next is None:
            return redirect("domainz-auth:register")
